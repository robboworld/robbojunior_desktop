...
let isDebugging = true; //modified_by_Yaroslav
let time;
let userStart = false;
let onHold = false;
...
export default class ScratchJr {
    ...
    static editorEvents () {
        ...
        window.onmousedown = ScratchJr.unfocus; //modified_by_Yaroslav
    ...
    static getSprite () {
//AZ
        if (!stage.currentPage || !stage.currentPage.currentSpriteName) {
            return undefined;
        }
    ...
    static displayStatus (type) {
        var ids = ['topsection', 'blockspalette', 'scripts', 'flip', 'projectinfo','robot_panel'];
