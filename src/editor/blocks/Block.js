...
export default class Block {
   ...
   cut_extension(file_name){
        ...
        for (let i = 0; i< extensions.length;i++){
            ...
            if (file_name.indexOf(extensions[i]) != -1 ){

                return file_name.replace(extensions[i],"");
            }
    ...
    setBlockshapeFromSpecs (spec, isPalette, scale) {
        ...
        this.icon = (this.blocktype == 'playsnd')?  BlockSpecs.getImageFrom('assets/blockicons/' + this.cut_extension(spec[4]), 'svg'): spec[1] ;
        this.image = spec[2];
