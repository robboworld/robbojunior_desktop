...
export default class Thumbs {
    ...
    static prepareToDragPage (e) {
        ...
        var my = Events.dragmousey - frame.offsetTop  - gn("pagecc").scrollTop - localy(Events.dragthumbnail, Events.dragmousey); /*- gn("pagescc").scrollTop -   modified_by_Yaroslav*/
        var mstyle = {
            position: 'absolute',
            left: '0px',
            top: '0px',
      ...
      //  Events.dragcanvas.style.webkitTransform = 'translate3d(' + mx + 'px,' + my + 'px, 0)';
        setProps(Events.dragcanvas.style, mstyle);
        Events.move3D_extended(Events.dragcanvas, mx, my /*- gn("pagecc").scrollTop*/ ,mx,my /*- gn("pagecc").scrollTop */); /*modified_by_Yaroslav*/
        frame.appendChild(Events.dragcanvas);
      //   gn("pagecc").appendChild(Events.dragcanvas);
        ...
        Thumbs.layoutPages();
        Events.dragthumbnail.pos = Thumbs.getPagePos(Events.dragcanvas.top + gn("pagecc").scrollTop); //modified_by_Yaroslav
    }
    ...
    static draggingPage (e, el) {
        ...
        Events.move3D_extended(el, dx, dy /*- gn("pagecc").scrollTop*/,dx, dy/* - gn("pagecc").scrollTop*/); /*modified_by_Yaroslav*/
        if (!caret) {
            return;
        }
        Thumbs.removeCaret();
    ...
    static insertCaret (el) {
        var pos = Thumbs.getPagePos(el.top + gn("pagecc").scrollTop);
