...
export default class Sprite {
    ...
    doRender (whenDone) {
        ...
        this.watermark = SVGTools.getWatermark(this.svg, '#D5D3D3'); // svg for watermark //#B3B3B3
        if (whenDone) {
            whenDone(this);
        }
    }
  ...
  /*  render () {
    ...
    render () {//modified_by_Yaroslav

        var dx, dy, mtx;

         let normalize_scale_x = this.scale;  //(100 / this.w);
         let normalize_scale_y = this.scale; // (100 / this.h);
