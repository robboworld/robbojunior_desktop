...
export default class Events {
    ...
    static move3D_extended (el, dx, dy,sx,sy) {
        if (!el) {
            return;
        }
        var mtx = new WebKitCSSMatrix(window.getComputedStyle(el).webkitTransform);
        el.top = dy + mtx.m42;
        el.left = dx + mtx.m41;
        el.style.webkitTransform = 'translate3d(' + (sx + mtx.m41) + 'px,' +  (sy + mtx.m42) + 'px, 0)';

        //el.style.webkitTransform = 'translate3d(' + (1400 + mtx.m41) + 'px,' +  (0 + mtx.m42) + 'px, 0)';
    }
