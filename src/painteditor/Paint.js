...
//AZ
    ...
    static mouseDown (e) {
        ...
        if (hitRect(gn('donecheck'), pt)) {
            Paint.backToProject(e);
        }else if (hitRect(gn('editcancel'), pt)){

              Paint.cancelImageEdit(e); //modified_by_Yaroslav
  ...
  static cancelImageEdit(e){
   ...
   Paint.close();
    ...
    static topbar () {
        ...
        Paint.checkMark(pt);
        Paint.cancelMark(pt); //modified_by_Yaroslav
        PaintUndo.setup(pt); // plug here the undo
        Paint.nameOfcostume(pt);
    }


   static  cancelMark(pt){ //modified_by_Yaroslav

     var clicky = newHTML('div', 'editcancel', pt);
     clicky.id = 'editcancel';
     if (isTablet) {
         clicky.ontouchstart = Paint.cancelImageEdit;
     } else {
         clicky.onmousedown = Paint.cancelImageEdit;
     }
...
//AZ
    ...
    */
    ...
    static createCharFromXML (str) {
      ...
      //  if ((dx + workspaceWidth) > 432 ) dx = 0;
      //  if ((dy + workspaceHeight) > 384 ) dy = 0;
